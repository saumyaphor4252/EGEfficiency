...
//#include "RecoTracker/MeasurementDet/src/TkMeasurementDetSet.h" //ADDED
//#include "RecoTracker/MeasurementDet/interface/MeasurementTrackerEvent.h"//ADDED
...
TrajSeedMatcher::Configuration::Configuration(const edm::ParameterSet& pset, edm::ConsumesCollector&& cc)
    : magFieldToken{cc.esConsumes()},
      ...
      minNrHitsValidLayerBins{pset.getParameter<std::vector<int> >("minNrHitsValidLayerBins")},
								    //badPixelFEDChannelCollectionTags{pset.getParameter<std::vector<edm::InputTag> >("badPixelFEDChannelCollectionLabels")},
...
bool TrajSeedMatcher::layerHasValidHits(const DetLayer& layer,
  ...
  if (detWithState.empty())
    ...
    //auto pixData = std::make_unique<PxMeasurementDetSet>(measTkEvt_.pixelData());
    //const PxMeasurementDetSet pixData = measTkEvt_.pixelData();
    //const auto pixData = measTkEvt_.pixelData();
