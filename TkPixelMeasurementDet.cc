...
bool TkPixelMeasurementDet::hasBadComponents(const TrajectoryStateOnSurface& tsos,
  ...
  for (auto const& broc : badRocPositions_) {
    ...
    if ((dx * dx < 9.f * 0.01) && (dy * dy < 9.f * 0.01))
      return true;
  }
  ...
  float dx = 3.f * std::sqrt(0.0001) + theRocWidth, dy = 3.f * std::sqrt(0.0001) + theRocHeight;
  for (auto const& p : *badFEDChannelPositions) {
    //std::cout << "p.first.x: " << p.first.x() << "   p.second.x: " << p.second.x() << std::endl;
    if (lp.x() > (p.first.x() - dx) && lp.x() < (p.second.x() + dx) && lp.y() > (p.first.y() - dy) &&
        lp.y() < (p.second.y() + dy)) {
      //std::cout << "TRUE SOMETIMES" << std::endl;
      return true;
    }
  }
